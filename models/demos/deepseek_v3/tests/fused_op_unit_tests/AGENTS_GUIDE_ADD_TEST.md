...
0. Check if the unit test already exists! If a unit test for the given op/ sequence of ops already exists, use the following list as a checklist to verify that all steps have been completed successfully. Complete all steps that have not been completed before and do all verification steps.
...
5. In the fused op unit test file, create a function "NEW_FUSED_OP_ttnn" calling a function in the module code containing the sequence of ttnn ops that correspond to the new fused op. If there is no function in the module code that already contains the sequence of ops, then create a function there and call it from the unit test; If it already esists just call it from the unit test. Inputs and outputs correspond to the inputs/output of the sequence of ops for the new fused op; inputs/outputs should be ttnn tensors and function parameters.
6. In the fused op unit test file, implement a pytest that:
    ...
    2. Runs 100 iterations and ckecks/asserts PCC and ATOL (on the last iteration)
...
10. Add a single device test for device performance, see step 10 for how to do that.
11. If single device tests are not skipped, *verify* the single device tests by running the single device, device perf test that generated the csv file and compare it to the multi device perf csv. All shapes must match, create a helper script to verify that.
12. Add a row in the table in models/demos/deepseek_v3/tests/fused_op_unit_tests/README.md for the newly added test and fill in the cells; read the instructions in the readme for interpretation of the features.
13. Do one final run of all tests in the test file and create a csv summary of results named like the test file with _results.csv in the end. Use the following structure for the csv: test_name,status,pcc,perf,failure_reason (optional),comment (optional), link. Add one line per concrete test (parameter combination) with all details filled in. In "comment" column add any comments for potential fixes is the test is failing or any other comments of interest. In "link" column add a link to the test log for the run.
