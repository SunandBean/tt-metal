...
def _skip_single_device_ccl():
    pytest.skip("Single-device test is not applicable because ds_moe_all_gather includes CCL ops.")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("prefill", 1024, 1.0, 0.2, 0.2, 0.0),
        ("prefill", 8192, 1.0, 0.2, 0.2, 0.0),
        ("prefill", 32768, 1.0, 0.2, 0.2, 0.0),
        ("prefill", 131072, 1.0, 0.2, 0.2, 0.0),
    ],
)
...
def test_ds_moe_all_gather(
    ...
    if trace_mode and not program_cache_enabled:
        pytest.skip("Trace mode requires program cache to be enabled.")
...
@pytest.mark.parametrize("program_cache_enabled", [True, False], ids=["program_cache", "no_program_cache"])
@pytest.mark.parametrize("trace_mode", [False, True], ids=["eager", "trace"])
@pytest.mark.parametrize("use_real_weights", [True], ids=["real_weights"])
@pytest.mark.parametrize(
    "device_params",
    [
        {
...
def test_ds_moe_all_gather_single_device(
    mode,
    seq_len,
    expected_pcc,
    expected_atol,
...
def test_ds_moe_all_gather_single_device_device_perf(mode, seq_len):
    _skip_single_device_ccl()
